...
class LearnLLMService:
    ...
    async def generate_flashcards_response(
        ...
        # VALIDATION: Ensure exactly 10 cards
        for action in board_actions:
            if action.kind == BoardActionKind.SHOW_FLASHCARDS:
                cards = action.payload.get("cards", [])
                if len(cards) != 10:
                    logger.warning(f"LLM generated {len(cards)} cards instead of 10. Attempting to truncate or log.")
                    if len(cards) > 10:
                        action.payload["cards"] = cards[:10]
                break
...
REQUIREMENTS:
1. **EXTRACT EXACTLY 10 CARDS**: You MUST extract exactly 10 distinct technical concepts or terms. No more, no less.
2. **FORMAT**: Return each card as a 'SHOW_FLASHCARDS' board action.
3. **STYLE**: 
   - Front: The concept name (short, max 5 words).
...
5. **QUANTITY IS CRITICAL**: If you find fewer than 10 concepts, drill into sub-concepts or specific parameters to reach exactly 10.
...
"""
    ...
    async def _call_token_factory(self, prompt: str) -> str:
        ...
        try:
            ...
            # Important: if it's a parsing error, we want to see the RAW content if possible
            try:
                # If we had access to the raw response object here, we'd log it
                pass
            except:
                pass
            return self._mock_response(getattr(self, '_current_student_input', ''))
    ...
    def _parse_response(self, raw_response: str) -> tuple[str, List[BoardAction]]:
        ...
        try:
            ...
            try:
                ...
                logger.info(f"Initial JSON parse failed: {initial_err}. Attempting individual action extraction...")
                # Attempt advanced extraction: find all objects {...} if the array itself is broken
                # This handles cases where the LLM might have output: [ {...}, {...} missing quotes here ]
                objects = re.findall(r'(\{(?:[^{}]|(?:\{[^{}]*\}))*\})', board_json)
                if objects:
                    raw_actions = []
                    for obj_str in objects:
                        try:
                            raw_actions.append(json.loads(obj_str))
                        except:
                            # Try basic repair on individual object
                            try:
                                fixed_obj = re.sub(r'([{,]\s*)([a-zA-Z_][a-zA-Z0-9_]*)\s*:', r'\1"\2":', obj_str)
                                fixed_obj = re.sub(r':\s*([a-zA-Z_][a-zA-Z0-9_À-ÿ\s]*)\s*([,}])', r': "\1"\2', fixed_obj)
                                raw_actions.append(json.loads(fixed_obj))
                            except:
                                continue
                else:
                    # Last ditch effort for minor syntax errors
