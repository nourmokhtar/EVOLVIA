...
from app.db.supabase import delete_record, get_supabase, query_table, insert_table, update_table
...
class LearnSession:
    ...
    def resume(self) -> None:
        ...
        # Idempotent resume: if already active, just return
        if self.status in [SessionStatus.TEACHING, SessionStatus.ANSWERING, SessionStatus.RESUMING]:
            logger.info(f"Resume called but status is already {self.status}. Ignoring.")
            return

        if self.status != SessionStatus.PAUSED:
            logger.warning(f"Attempted to resume from invalid state {self.status}")
            return
        self.status = SessionStatus.RESUMING

    def continue_teaching(self) -> None:
        """Continue teaching after resuming"""
        if self.status == SessionStatus.TEACHING:
            return
...
class LearnSessionManager:
    """
    Manages all learning sessions.
    """

    def __init__(self):
        self.supabase = get_supabase()

    async def get_session_by_id(self, session_id: str) -> Optional[LearnSession]:
        session_metadata = await query_table("session_metadata", {"id": session_id})
        if not session_metadata:
            return None
        session_metadata = session_metadata[0]
        return LearnSession(
            id=session_metadata["id"],
            status=SessionStatus(session_metadata["status"]),
            started_at=session_metadata["started_at"],
            updated_at=session_metadata["updated_at"],
        )

    async def start_session(self, session_id: str) -> LearnSession:
        ...
        await insert_table("session_metadata", session_metadata.dict())
        return LearnSession(
            id=session_id,
    ...
    async def update_session(self, session: LearnSession) -> LearnSession:
        await update_table("session_metadata", {"id": session.id}, session.dict(exclude_none=True))
        return session

    async def delete_session(self, session_id: str) -> None:
        await delete_record("session_metadata", {"id": session_id})

    async def list_sessions(self) -> list[LearnSession]:
        """List all active sessions"""
        sessions_metadata = await query_table("session_metadata", {})
        sessions = []
        if sessions_metadata:
            for session_meta in sessions_metadata:
                # Reconstruct LearnSession from minimal metadata
                session = LearnSession(
                    session_id=session_meta.get("id", "unknown"),
                    lesson_id=session_meta.get("lesson_id", "unknown"),
