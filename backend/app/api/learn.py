        ...
        return [
            {
                ...
                "progress": s.progress,
                "turns": len(s.history) if s.history else 0,
            }
            for s in sessions
        ]
    except Exception as e:
...
async def websocket_endpoint(websocket: WebSocket, session_id: str):
    ...
    if not exists:
        ...
        try:
            await websocket.accept() # Must accept before closing with custom code in some versions/browsers
    ...
    session = session_manager.get_session(session_id)
    logger.info(f"📦 Session state: {session.status if session else 'NONE'}")
    ...
    try:
        ...
        try:
            # VoiceManager only takes sample_rate, not session_id
            voice_manager = VoiceManager(sample_rate=16000)
            # await voice_manager.start() # Removed: VoiceManager has no start method
        except Exception as e:
            logger.error(f"Failed to initialize VoiceManager: {e}")
            voice_manager = None

    except Exception as e:
        logger.error(f"❌ WebSocket connection failed during accept: {e}")
        return

    logger.info(f"🚀 WebSocket connected and ready for session {session_id}")
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                logger.warning(f"WebSocket receive failed (likely closed): {e}")
                break

            # Handle disconnect message
            if msg["type"] == "websocket.disconnect":
                logger.info(f"👋 Received disconnect message for session {session_id}")
                break
            ...
            if "text" in msg:
                ...
                if event_type == "USER_MESSAGE":
                    ...
                    if not voice_manager:
                        await _send_event(websocket, ErrorEvent(session_id=session_id, error_code="NO_VOICE", message="Voice system unavailable"))
                        continue
                ...
                if voice_manager:
                    ...
                    if transcribed_text:
                        ...
                        # Send transcription back to frontend so it can be displayed in input bar
                        # and sent as a regular message (as requested by user)
                        await _send_event(
                            websocket,
                            VoiceTranscriptionEvent(
                                session_id=session_id,
                    ...
                    # Ignore audio if voice system not active
                    pass
    
    except WebSocketDisconnect:
        logger.info(f"👋 WebSocket disconnected (WebSocketDisconnect) for session {session_id}")
    except Exception as e:
        # Avoid logging the "receive" error during cleanup
        if "receive" in str(e) and "disconnect" in str(e):
            logger.info(f"ℹ️ WebSocket connection cleaned up for {session_id}")
        else:
        ...
        try:
            # Final attempt to notify error if still open
        ...
        logger.info(f"🏁 WebSocket handler terminated for session {session_id}")
