...
def heuristic_pronunciation(
    ...
    # Add general fallback issue if score is low but no specific patterns matched
    overall_score = _clamp(avg_conf * 100.0, 0.0, 100.0)
    
    if not issues and overall_score < 75:
        issues.append(
            {
                "label": "General Clarity",
                "description": "The system detected lower confidence in some parts of your speech. Focus on articulating clearly and maintaining a steady volume.",
                "certainty": "likely",
            }
        )
